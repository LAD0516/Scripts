...
[rewrite_local]
#获取挖矿②Cookie
^https:\/\/operation-api\.jimistore\.com\/* url script-request-body zfbwk2.js
...
function setSignData() {
  if ($request.url.indexOf("createSign") > -1) {
    $.setdata($request.url,'ttwk2url')
    $.setdata(JSON.stringify($request.headers),'ttwk2header')
    $.setdata($request.body,'ttwk2body')
 ...
 function createSign(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let url = {
        url : $.getdata('ttwk2url'),
        headers : JSON.parse($.getdata('ttwk2header')),
        body : $.getdata('ttwk2body')
      }
...
function goHelp(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let url = {
        url : $.getdata('ttwk2url').replace('sign/createSign','help/goHelp').replace('v2','v1'),
        headers : JSON.parse($.getdata('ttwk2header')),
        body : JSON.stringify({"targetUserId":"8a28a661752b378701754d2ec1b31711","currentUserId":JSON.parse($.getdata('ttwk2header')).userid})
      }
      $.post(url, async (err, resp, data) => {
        try {
          //console.log(data)
...
function showSignInfo(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      if (typeof $.getdata('ttwk2url') === "undefined") {
        $.msg($.name,"",'请先启用获取Cookie脚本然后点击此通知！', openurl)
        return
      }
      let url = {
        url : $.getdata('ttwk2url').replace('createSign','showSignInfo').replace('v2','v1'),
        headers : JSON.parse($.getdata('ttwk2header'))
      }
      $.post(url, async (err, resp, data) => {
        try {
