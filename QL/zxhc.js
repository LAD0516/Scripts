...
let userCookie = ($.isNode() ? process.env.zxhcCookie : $.getdata('zxhcCookie')) || '';
let userList = []
let userCookieArr = [];
let userIdx = 0
let userCount = 0
...
class UserInfo {
    constructor(str) {
        ...
        this.cid = this.cookie.match(/cid=(.+?);/)[1]
        this.auth = this.cookie.match(/auth=(.+?);/)[1]
        this.id = this.cookie.match(/deviceId=(.+?);/)[1]
        this.token = this.cookie.match(/deviceToken=(.+?);/)[1]
    }
   ...
   async login() {
	...
	return new Promise((resolve) => {
            let url = {
        url: `https://m.suanya.com/restapi/soa2/18903/json/attendanceDay?_fxpcqlniredt=${this.cid}&x-traceID=${this.cid}-${time}-9113800`,
        headers: {
'Origin' : `https://pages.suanya.com`,
'Accept-Encoding' : `gzip, deflate, br`,
'Connection' : `keep-alive`,
'Content-Type' : `application/json`,
'Accept' : `*/*`,
'Host' : `m.suanya.com`,
'User-Agent' : `Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_2 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Mobile/11B508_Tieyou_TieyouWireless_9.7.7_partner_zhixing_cDevice=iPhone XR_isPhoneX=1__cSize=w414*h896_isiPhoneX`,
'Referer' : `https://pages.suanya.com/`,
'Accept-Language' : `zh-CN,zh-Hans;q=0.9`
},
        body: `{"head":{"cid":"${this.cid}","ctok":"","cver":"707.977","lang":"01","sid":"4505","syscode":"12","auth":"${this.auth}","xsid":"","extension":[{"name":"platform","value":"ios"},{"name":"reqTime","value":"${time}"},{"name":"clientInfo","value":"ios|Apple_iPhone XR|15.0|9.7.7"},{"name":"appVersion","value":"9.7.7"},{"name":"deviceId","value":"${this.id}"},{"name":"partner","value":"zhixing"},{"name":"channel","value":"ZXI"},{"name":"sign","value":""},{"name":"deviceToken","value":"${this.token}"},{"name":"latitude","value":""},{"name":"longitude","value":""},{"name":"appid","value":"1003"},{"name":"correctVersion","value":"1156"},{"name":"sourceType","value":"app"}],"sauth":""},"sourceType":0,"type":1}`,
    };
		...
		console.log(data+"\n")}
            ...
            msg += `账号【${this.index}】签到成功！\n`
            
				} else {
            console.log(`账号【${this.index}】签到失败：${result.resultMessage}`)
            msg += `账号【${this.index}】签到失败：${result.resultMessage}\n`
}
            await $.wait(500);
            await this.cx();
	...
	return new Promise((resolve) => {
            let url = {
        url: `https://m.suanya.com/restapi/soa2/20707/json/GetCreditDeductionProducts?_fxpcqlniredt=${this.cid}&x-traceID=${this.cid}-${time}-8607626`,
        headers: {
'Origin' : `https://pages.suanya.com`,
'Accept-Encoding' : `gzip, deflate, br`,
'Connection' : `keep-alive`,
...
'Accept-Language' : `zh-CN,zh-Hans;q=0.9`
	 ...
	 body: `{"head":{"cid":"${this.cid}","ctok":"","cver":"707.977","lang":"01","sid":"4505","syscode":"12","auth":"${this.auth}","xsid":"","extension":[{"name":"platform","value":"ios"},{"name":"reqTime","value":"${time}"},{"name":"clientInfo","value":"ios|Apple_iPhone XR|15.0|9.7.7"},{"name":"appVersion","value":"9.7.7"},{"name":"deviceId","value":"${this.id}"},{"name":"partner","value":"zhixing"},{"name":"channel","value":"ZXI"},{"name":"sign","value":""},{"name":"deviceToken","value":"${this.token}"},{"name":"latitude","value":""},{"name":"longitude","value":""},{"name":"appid","value":"1003"},{"name":"correctVersion","value":"1156"},{"name":"sourceType","value":"app"}],"sauth":""},"sourceType":0,"type":1}`,
		...
		console.log(data+"\n")}
            ...
            if(!result) return
            //console.log(result)
            this.jf =result.userCredit
            if(result.resultCode == 1) {
                console.log(`账号【${this.index}】现有积分：${this.jf}`)
                msg += `账号【${this.index}】现有积分：${this.jf}\n`
        } else {
            console.log(`账号【${this.index}】查询失败：${result.resultMessage}`)
            msg += `账号【${this.index}】查询失败：${result.resultMessage}\n`
...
async function GetRewrite() {
    if($request.url.indexOf(`GetCreditDeductionProducts`) > -1) {
        let ck = $request.body
        
        if(userCookie) {
            if(userCookie.indexOf(ck) == -1) {
                userCookie = userCookie + '@' + ck
                $.setdata(userCookie, 'zxhcCookie');
