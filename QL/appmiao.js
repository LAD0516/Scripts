...
#APP喵-签到
cron: 34 9,21 * * *
export appmiaoCookie='username=你的账号; password=你的密码;'
*/
const $ = new Env('APP喵签到');
const jsname = 'APP喵签到'
const logDebug = 0
...
let userCookie = ($.isNode() ? process.env.appmiaoCookie : $.getdata('appmiaoCookie')) || '';
let userList = []
...
class UserInfo {
    ...
    async login() {
        ...
        let url = `https://www.appmiu.com/wp-json/jwt-auth/v1/token`
    ...
    async getUserMission() {
        let url = `https://www.appmiu.com/wp-json/b2/v1/getUserMission`
    ...
    async doSign() {
        let url = `https://www.appmiu.com/wp-json/b2/v1/userMission`
        let body = ``
        let urlObject = populateUrlObject(url,this.token,body)
        await httpRequest('post',urlObject)
        let result = httpResult;
