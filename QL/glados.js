 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export GLaDOS_ck=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
...
//////////////////////
let GLaDOS_ck = process.env.GLaDOS_ck;               // 这里是 从青龙的 配置文件 读取你写的变量
let GLaDOS_ckArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		for (let index = 0; index < GLaDOS_ckArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = GLaDOS_ckArr[index].split('&');      // 这里是分割你每个账号的每个小项   
...
async function Envs() {
	if (GLaDOS_ck) {
		if (GLaDOS_ck.indexOf("@") != -1) {
			GLaDOS_ck.split("@").forEach((item) => {
				GLaDOS_ckArr.push(item);
			});
		} else {
			GLaDOS_ckArr.push(GLaDOS_ck);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 GLaDOS_ck`)
		return;
	}
